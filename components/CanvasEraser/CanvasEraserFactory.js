const DEFAULT_OPTIONS = {
  background: '#000',
  completeRatio: 1,
  enabled: true,
  onComplete: null,
  onProgress: null,
  size: 40,
};

const getElementComputedStyled = (element, prop) =>
  parseFloat(getComputedStyle(element, null)[prop].replace('px', ''));

const factory = () => {
  let _canvas = null;
  let _context = null;
  let _data = {};

  const _handleEraserProgress = (currentX, currentY) => {
    const {
      colParts,
      numParts,
      completeRatio,
      enabled,
      size,
      onComplete,
      onProgress,
    } = _data;

    if (!enabled) {
      console.log('⚠️ Canvas Eraser: Disabled, skipping progress');
      return;
    }

    let p =
      Math.floor(currentX / size) + Math.floor(currentY / size) * colParts;

    if (p >= 0 && p < numParts) {
      _data.ratio += _data.parts[p];
      _data.parts[p] = 0;

      if (!_data.complete) {
        p = _data.ratio / _data.numParts;

        if (p >= completeRatio) {
          _data.complete = true;
          console.log('✅ Canvas Eraser: Erasing completed! Progress: 100%');
          if (onComplete) {
            onComplete();
          }
          return;
        }

        if (onProgress) {
          onProgress(p);
          if (Math.random() < 0.05) { // Log progress occasionally (5% of the time)
            console.log('📊 Canvas Eraser: Progress update', (p * 100).toFixed(1) + '%');
          }
        }
      }
    }
  };

  const _onMouseMove = event => {
    event.preventDefault();

    const { enabled, posX, posY, scaleRatio, touchX, touchY } = _data;
    const currentX = (event.pageX - posX) * scaleRatio;
    const currentY = (event.pageY - posY) * scaleRatio;

    if (enabled) {
      console.log('🖌️ Canvas Eraser: Erasing at position', { 
        x: currentX.toFixed(2), 
        y: currentY.toFixed(2),
        progress: (_data.ratio / _data.numParts * 100).toFixed(1) + '%'
      });
      _handleEraserProgress(currentX, currentY);
      _context.beginPath();
      _context.moveTo(touchX, touchY);
      _context.lineTo(currentX, currentY);
      _context.stroke();
    }

    _data.touchX = currentX;
    _data.touchY = currentY;
  };

  const _onMouseEnter = event => {
    event.preventDefault();
    console.log('🎨 Canvas Eraser: Mouse entered canvas boundary');

    const { enabled, posX, posY, scaleRatio } = _data;
    const currentX = (event.pageX - posX) * scaleRatio;
    const currentY = (event.pageY - posY) * scaleRatio;

    console.log('🎨 Canvas Eraser: Enter position', { currentX, currentY, enabled });

    _data.touchX = currentX;
    _data.touchY = currentY;
  };

  const _onMouseLeave = () => {
    console.log('🎨 Canvas Eraser: Mouse left canvas boundary');
  };

  const _onMouseDown = event => {
    event.preventDefault();

    const { enabled, posX, posY, scaleRatio } = _data;
    const currentX = (event.pageX - posX) * scaleRatio;
    const currentY = (event.pageY - posY) * scaleRatio;

    _data.touchDown = true;
    _data.touchX = currentX;
    _data.touchY = currentY;

    if (enabled) {
      _handleEraserProgress(currentX, currentY);

      _context.beginPath();
      _context.moveTo(currentX - 1, currentY);
      _context.lineTo(currentX, currentY);
      _context.stroke();
    }

    _canvas.addEventListener('mousemove', _onMouseMove);
  };

  const _onMouseClick = event => {
    event.preventDefault();

    const { enabled, posX, posY, scaleRatio } = _data;
    const currentX = (event.pageX - posX) * scaleRatio;
    const currentY = (event.pageY - posY) * scaleRatio;

    _data.touchDown = true;
    _data.touchX = currentX;
    _data.touchY = currentY;

    if (enabled) {
      _handleEraserProgress(currentX, currentY);

      _context.beginPath();
      _context.moveTo(currentX - 1, currentY);
      _context.lineTo(currentX, currentY);
      _context.stroke();
    }

    _canvas.addEventListener('mousemove', _onMouseMove);
  };

  const init = (source, options = {}) => {
    console.log('🎨 Canvas Eraser: Initializing', { options });
    
    if (!source) {
      console.error('❌ Canvas Eraser: No source element provided!');
      throw new Error(
        'No source element provided. It must be an HTML canvas element.',
      );
    }

    const currentOptions = { ...DEFAULT_OPTIONS, ...options };
    const { size, background } = currentOptions;
    console.log('🎨 Canvas Eraser: Options set', { size, background });

    _canvas = source;
    _context = _canvas.getContext('2d');
    const devicePixelRatio = window.devicePixelRatio || 1;
    const backingStoreRatio =
      _context.webkitBackingStorePixelRatio ||
      _context.mozBackingStorePixelRatio ||
      _context.msBackingStorePixelRatio ||
      _context.oBackingStorePixelRatio ||
      _context.backingStorePixelRatio ||
      1;
    const scaleRatio = devicePixelRatio / backingStoreRatio;

    const realWidth = getElementComputedStyled(_canvas, 'width');
    const realHeight = getElementComputedStyled(_canvas, 'height');
    const width = realWidth * scaleRatio;
    const height = realHeight * scaleRatio;

    if (devicePixelRatio !== backingStoreRatio) {
      _canvas.width = width;
      _canvas.height = height;
      _canvas.style.width = `${realWidth}px`;
      _canvas.style.height = `${realHeight}px`;
    } else {
      _canvas.width = realWidth;
      _canvas.height = realHeight;
      _canvas.style.width = '';
      _canvas.style.height = '';
    }

    // _context.scale(scaleRatio, scaleRatio);
    _context.fillStyle = background;
    _context.fillRect(0, 0, width, height);
    _context.drawImage(_canvas, 0, 0, width, height);

    // prepare context for drawing operations
    _context.globalCompositeOperation = 'destination-out';
    _context.lineWidth = size;
    _context.lineCap = 'round';

    // bind events - add mousemove directly to always work
    _canvas.addEventListener('mousemove', _onMouseMove);
    _canvas.addEventListener('mouseenter', _onMouseEnter);
    _canvas.addEventListener('mouseleave', _onMouseLeave);
    console.log('🎨 Canvas Eraser: Event listeners attached (mousemove, mouseenter, mouseleave)');
    // _canvas.addEventListener('touchstart', _onTouchStart);
    // _canvas.addEventListener('touchmove', _onTouchMove);
    // _canvas.addEventListener('touchend', _onTouchEnd);

    // reset parts
    const parts = [];
    const colParts = Math.floor(width / size);
    const numParts = colParts * Math.floor(height / size);
    for (let i = 0; i < numParts; i++) {
      parts.push(1);
    }
    console.log('🎨 Canvas Eraser: Parts grid created', { colParts, numParts, totalParts: parts.length });

    _data = {
      posX: _canvas.offsetLeft,
      posY: _canvas.offsetTop,
      touchDown: false,
      touchID: -999,
      touchX: 0,
      touchY: 0,
      ptouchX: 0,
      ptouchY: 0,
      w: width,
      h: height,
      scaleRatio: scaleRatio,
      ratio: 0,
      complete: false,
      parts,
      colParts,
      numParts,
      currentOptions,
      ...currentOptions,
    };
    console.log('🎨 Canvas Eraser: Initialization complete', _data);
  };

  const clear = () => {
    console.log('🎨 Canvas Eraser: Clear called');
    const { w, h, numParts, onComplete } = _data;

    if (_data) {
      _context.clearRect(0, 0, w, h);

      for (let i = 0; i < numParts; i++) {
        _data.parts[i] = 0;
      }

      _data.ratio = numParts;
      _data.complete = true;
      console.log('🎨 Canvas Eraser: Cleared completely');

      if (onComplete) {
        console.log('🎨 Canvas Eraser: Calling onComplete callback');
        onComplete();
      }
    }
  };

  const reset = () => {
    console.log('🎨 Canvas Eraser: Reset called');
    const { w, h, numParts } = _data;

    _context.globalCompositeOperation = 'source-over';
    _context.drawImage(_canvas, 0, 0, w, h);
    _context.globalCompositeOperation = 'destination-out';

    for (let i = 0; i < numParts; i++) {
      _data.parts[i] = 1;
    }

    _data.ratio = 0;
    _data.complete = false;
    _data.touchDown = false;
    console.log('🎨 Canvas Eraser: Reset complete');
  };

  return {
    init,
    clear,
    reset,
  };
};

export default factory;
